...
It turns out that the model that gives "no global warming for 18 years" is broken, and hides a very amusing implication which I want to highlight.
To get the 18 warming-free years, it implicitly *crams 18 years' worth of warming into a single instantaneous jump*.

Happily, it is very easy to fix the model.
...
This data fluctuates -- a *lot*.
These fluctuations are actual physical events, but for our purposes they're just noise: we're looking for underlying *trends* in the data.
...
We get a slope of `r PrintTrend(single_trendline_slope)`.

This method is known as "[linear least squares](https://en.wikipedia.org/wiki/Linear_least_squares_(mathematics))".
It is exceedingly simple -- and that is its strength!
...
When we add them back in, we reveal a fatal flaw in his approach.
...
[^escalator]: This absurd temperature jump is implicit in other analyses (such as the excellent [escalator animation](http://www.skepticalscience.com/graphics.php?g=47)), but in this post I wanted to highlight it explicitly.
